...
var Main = function() {"use strict";
	...
	var runModuleTools = function() {
		...
		$('body').on('click', '.panel-expand', function(e) {
			...
			var panelEle = $(this).parent().parents('.panel');
			var panelHeadingTitle = panelEle.children('.panel-heading').children('.panel-title').children()[0];
			var title = panelHeadingTitle.innerText;
			var elebox = panelEle.children('.panel-body');
			var elehtml = elebox.html();
			var modalbox = '' +
				...
				'</div>'+
				'</div>';
			//var modalwboxdom = jQuery.parseHTML(modalwbox);
		...
		// Graph export
		$('body').on('click', '.panel-export-png', function(e) {
			//get .panel
			var panelEle = $(this).parent().parents('.panel');
			//get title
			var panelHead = panelEle.children(".panel-heading");
			var title = panelHead.children(".panel-title").children().html();
			//get svg
			var panelBody = panelEle.children(".panel-body");
			var svgE = panelBody.children();
			var widthSVG = svgE.width();
			var heightSVG = svgE.height();
			var html =  '<svg width="'+widthSVG+'" height="'+heightSVG+'" title="svg2png" version="1.0" xmlns="http://www.w3.org/2000/svg">'+ svgE.html()+'</svg>';

			var image = new Image();
			image.width = widthSVG;
			image.height = heightSVG;
			image.src = 'data:image/svg+xml;utf8,' + html;
		...
		// Data Export
		$('body').on('click', '.panel-export-csv', function(e) {
			var data = [
					{ "系统":"结算系统","缺陷统计":1000 },
					{ "系统":"前台UI","缺陷统计":1200 }
				];
            ...
            //
			//});
			var dataCSVstr = d3.csv.format(data);
			csvContent += dataCSVstr;
